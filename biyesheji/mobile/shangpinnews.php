...
<html lang="zh-CN">
  ...
  if(!$as){
 header("location:../pc/zuixingshangxian.php"); 
}
?>
<?php include("top.php");?>
    ...
    <div class="contain">
      ...
      <section class="mod-box">
        <div class="row mod-hd bdr">
          <h3 class="col-xs-10 mod-title">最新上线商品</h3>
        </div>
        <div class="row thumbnail-part">
         <?php
            //得到数据库中所有商品
           $sql="select * from shangpin where status like '%是%' and news like '是' ";
          ...
          $sql="select * from shangpin where status like '%是%' and news like '是' limit {$offset},{$pageSize}";
          if($sql){
            $rows=fetchAll($sql);
          }
          else{
