...
<div class="shangpin_3">
    ...
    <div class="shangpin_right" >
        ...
        <div class="shangpin3_right">
            <span class="shangpin3_right2">
            您当前的位置：<a href="index.php" >首页</a>>>
            <a href="shangpinall.php">商品全部</a>
            </span>
      </div>
        ...
        <?php
          ...
          if(!$cnamerows){  
                echo "<script>alert('对不起，没有该商品分类的内容！！！');
                      window.location='shangpinall.php';</script>";
               exit();
          }
          if (!$cnamerows) {
              echo "<script>alert('对不起，没有该商品分类的内容！！！');window.location='shangpinall.php';</script>";
          }
          if(!$rows){  
          ...
          if (!$rows) {
              echo "<script>alert('对不起，没有该商品的内容！！！');window.location='shangpinall.php';</script>";
          }
          // 如果搜搜没有相应的商品信息时
...
<script type="text/javascript">
  function search1(){
      var val=document.getElementById("search1").value;
            if (val.length>0) {
      window.location="shangpinall.php?keywords="+val;
            }else{
                alert("对不起，不能为空！！！请重新输入");
                window.location="shangpinall.php";
            }
  }
  function change(val){
    window.location="shangpinall.php?order="+val;
  }
    function change1(val){
        window.location="shangpinall.php?shangpincate="+val;
    }
</script>
</html>
</body>
