...
<body>
    ...
    require_once '../include.php';
          if($as){
           header("location:../mobile/sahngpinghot.php"); 
          }
    ?>
    <?php include("top.php");?>
<!-- 商品展示的第三部分 -->
<div class="shangpin_3">
	<div class="shangpin_3_left">
        ...
        <p class="asdf"><a href="shangpinall.php" onclick="up()"><img src="images/gonsi_2.gif" />商品全部 <!-- <font style="float:right">></font> --></a></p>
            ...
            <p class="asdf f"><a href="rexiao.php"><img src="images/gonsi_2.gif" />热销商品</a></p>
		</div>
</div>
    <script type="text/javascript">
	function up(){
    ...
    <?php
          ...
          $com="where p.status like '%是%' and p.hot like '%是%'";
          //得到数据库中所有商品
    ...
    ?>
     	<div class="shangpin3_right">
            <span class="shangpin3_right1">热销商品</span>
            <span class="shangpin3_right2">您当前的位置：<a href="index.php" >首页</a>>><a href="rexiao.php">热销商品</a></span>
        </div>
        <div class="shangpin3_right3 " id="shangpin">
          ...
          if(!$cnamerows){  
                echo "<script>alert('对不起，没有该商品分类的内容！！！');
                      window.location='rexiao.php';</script>";
               exit();
          }
          if (!$cnamerows) {
              echo "<script>alert('对不起，没有该商品分类的内容！！！');window.location='rexiao.php';</script>";
          }
           if(!$rows){  
          ...
          if (!$rows) {
              echo "<script>alert('对不起，没有该商品内容！！！');window.location='rexiao.php';</script>";
          }
          // 如果搜搜没有相应的商品信息时
...
<script type="text/javascript">
  function search1(){
      var val=document.getElementById("search1").value;
            if (val.length>0) {
      window.location="rexiao.php?keywords="+val;
            }else{
                alert("对不起，不能为空！！！请重新输入");
                window.location="rexiao.php";
            }
  }
  function change(val){
    window.location="rexiao.php?order="+val;
  }
    function change1(val){
        window.location="rexiao.php?shangpincate="+val;
    }
</script>
</html>
</body>
